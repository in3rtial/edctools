...
class LogWatcher(object):
    """Looks for changes in all files of a directory in real-time. 
       Supports log rotation.
    """
    __slots__ = ["folder", "_callback", "matcher", "_files_map", "_sizehint"]
    def __init__(self, folder, callback, 
                 matcher=lambda filename: filename.endswith(".log"),
        ...
        (callable) @callback:
            a function which is called every time one of the f ile being
            watched is updated;
            this is called with "filename" and "lines" arguments.

        (list) @matcher:
            function to filter the files to watch for (use regex, contains...)
        ...
        assert callable(matcher), repr(matcher)
    ...
    def listdir(self):
        ...
        files_in_dir = [x for x in ls if self.matcher(x)]
        return files_in_dir
